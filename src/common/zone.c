...
{
	Hunk_Print ((Cmd_Argc() < 2) ? false : true);
}
...
void Cache_Print (void)
{
	...
	Com_Printf ("-------------------------\n");
	Com_Printf ("%8i total cache size\n", totalsize);
}
...
void Memory_Init (int hunksize)
{
	...
	if(!hunk_base)
		Sys_Error ("Memory_Init: alloc failed!");
	hunk_size = hunksize;
	hunk_low_used = 0;
	hunk_high_used = 0;
	...
	Cmd_AddCommand ("cachelist", Cache_Print_f);
	Cmd_AddCommand ("hunklist", Hunk_Print_f);
	Cmd_AddCommand ("zonelist", Z_Print_f);
