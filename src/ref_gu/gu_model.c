...
*/
// gu_models.c -- model loading and caching
...
model_t *Mod_LoadModel (model_t *mod, qboolean crash)
{
	...
	default:
		ri.Sys_Error (ERR_DROP,"Mod_LoadModel: unknown fileid for %s", mod->name);
		break;
	}
...
void Mod_LoadLighting (lump_t *l)
{
	...
	loadmodel->lightdata = ri.Hunk_AllocName (l->filelen, loadname);
...
void Mod_LoadEdges (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		ri.Sys_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = ri.Hunk_AllocName ((count + 1) * sizeof(*out), loadname);
...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		out->image = GL_FindImage (name, IMG_TYPE_WALL);
		if (!out->image)
		{
...
void Mod_LoadPlanes (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		ri.Sys_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = ri.Hunk_AllocName (count * 2 * sizeof(*out), loadname);
