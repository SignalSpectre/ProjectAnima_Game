...
// draw the background
	...
	Com_sprintf (version, sizeof(version), "%s v%s", APP_NAME, APP_VERSION);
	version_length = strlen(version);
	for (x = 0; x < version_length; x++)
		re.DrawChar (viddef.width - ((version_length + 1 - x) << 3), lines-12, 128 + version[x]);
