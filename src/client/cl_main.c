...
void CL_WriteDemoMessage (void)
{
	...
	FS_FWrite (cls.demofile, &swlen, 4);
	FS_FWrite (cls.demofile, net_message.data + 8, len);
}
...
void CL_Record_f (void)
{
	...
	cls.demofile = FS_FOpen (name, FS_MODE_WRITE | FS_PATH_GAMEDIR);
	if (!cls.demofile)
	{
	...
	for (i=0 ; i<MAX_CONFIGSTRINGS ; i++)
	{
		if (cl.configstrings[i][0])
		{
			...
			{	// write it out
				...
				FS_FWrite (cls.demofile, &len, 4);
				FS_FWrite (cls.demofile, buf.data, buf.cursize);
				buf.cursize = 0;
			}
	...
	if (precache_check >= CS_MODELS && precache_check < CS_MODELS+MAX_MODELS)
	{
		if (allow_download_models->value)
		{
			while (precache_check < CS_MODELS+MAX_MODELS && cl.configstrings[precache_check][0])
			{
				...
				if (!precache_model)
				{
					precache_model = FS_LoadFile (cl.configstrings[precache_check], NULL, FS_PATH_ALL | FS_FLAG_MTEMP);
					if (!precache_model)
					{
						precache_model = 0;
				...
				while (precache_model_skin - 1 < LittleLong(pheader->num_skins))
				{
					if (!CL_CheckOrDownloadFile((char *)precache_model +
						LittleLong(pheader->ofs_skins) + (precache_model_skin - 1)*MAX_SKINNAME))
					{
						precache_model_skin++;
						return; // started a download
					}
...
void CL_WriteConfiguration (void)
{
	...
	Com_sprintf (path, sizeof(path), "config.cfg");
	f = FS_FOpen (path, FS_MODE_WRITE | FS_PATH_GAMEDIR | FS_FLAG_TEXT);
	if (!f)
	{
	...
	FS_FPrintf (f, "// generated by quake, do not modify\n");
	Key_WriteBindings (f);
	FS_FClose (f);
